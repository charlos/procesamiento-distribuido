...
int fs_handshake(int server_socket, char type, char * node_name, char * node_ip_port, int blocks, t_log * logger) {
	...
	uint8_t prot_ope_code = sizeof(uint8_t);
	uint8_t prot_type = sizeof(char);
	uint8_t prot_node_name_size = sizeof(uint8_t);
	uint8_t prot_node_ip_port_size = sizeof(uint8_t);
	uint8_t prot_blocks = sizeof(uint32_t);
...
int fs_upload_file(int server_socket, char * path, char type, int buffer_size, void * buffer, t_log * logger) {
	...
	/**	╔═════════════════════════╦═════════════════════╦══════╦═══════════════╦════════════════════════╦════════╗
    	...
    	╚═════════════════════════╩═════════════════════╩══════╩═══════════════╩════════════════════════╩════════╝ **/
	uint8_t prot_ope_code = sizeof(uint8_t);
	uint8_t prot_path_size = sizeof(uint8_t);
	uint8_t prot_type = sizeof(char);
	uint8_t prot_buffer_size = sizeof(uint32_t);

	uint8_t  req_ope_code = UPLOAD_FILE;
	uint8_t req_path_size = strlen(path) + 1;
	char req_ope_type = type;
...
t_fs_get_md_file_resp * fs_get_metadata_file(int server_socket, char * path, t_log * logger) {
	...
	uint8_t resp_prot_file_size = sizeof(uint32_t);
	...
	uint8_t resp_prot_file_type = sizeof(char);
	...
	uint8_t resp_prot_blocks = sizeof(uint32_t);
	int blocks;
	...
	uint8_t resp_prot_block_copies = sizeof(uint32_t);
